...
brm.pm25 <- brm(Covid.Deaths ~ offset(log(Ecovid)) + a1524+a2534+a3544+a4554+a5564+over65PE + pm25avg + (1|ST_ABBR),
              data = brmsDF, 
              family = "poisson",chains=8,
              prior = prior,
              # family = poisson(),
              iter = niter, cores  = 16,seed = 2020,
...
```{r}
sirMap<-as.data.frame(uscounty)
# sirMap <- sirMap[complete.cases(sirMap[, c("Covid.Deaths",
#                                            "clrd.SMR",
#                                            # "ckd.SMR",
#                                            "diabetes.SMR",
#                                            "hta.SMR",
...
sirMap$re_v <- 1:nrow(sirMap)

f1<-Covid.Deaths ~ clrd.SMR + a1524+a2534+a3544+a4554+a5564+over65PE + pm25avg + EP_POV + blackPE+latinoPE + malePE+ femalePE  + diabetes.SMR + hta.SMR + ischemic.SMR + avgObesityPrev + f(ST_ABBR,model = "iid") + f(re_v, model = "iid")
inla.cv19.smr <- inla(f1,
                 family = "zeroinflatedpoisson1",
                 # family = "poisson",
                 data = sirMap,E = Ecovid,
                 num.threads = 8,verbose = F,
                 control.predictor = list(compute = TRUE),
                 control.compute = list(dic = TRUE,waic=T,cpo=TRUE))

sirMap$RRcv19smr<- inla.cv19.smr$summary.fitted.values[, "mean"]
sirMap$LLcv19smr <- inla.cv19.smr$summary.fitted.values[, "0.025quant"]
sirMap$ULcv19smr <- inla.cv19.smr$summary.fitted.values[, "0.975quant"]
summary(inla.cv19.smr)
round(exp(inla.cv19.smr$summary.fixed[,c(1,3,5)]),2)
bri.hyperpar.plot(inla.cv19.smr)
bri.Pois.resid(inla.cv19.smr)
tab <- inla.cv19.smr$summary.random$ST_ABBR
...
Black population has 2% higher risk of covid-19, pm2.5 has 
